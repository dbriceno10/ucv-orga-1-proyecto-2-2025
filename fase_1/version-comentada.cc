...
// Estructuras para manejar las líneas de código y las instrucciones
struct datosLinea {
    unsigned short dir;
...
// Función para obtener el número de un registro
...
// Función para buscar una etiqueta en la tabla de símbolos
...
// Función para convertir texto a número (soporta hexadecimales y etiquetas)
...
// Función para separar una línea en tokens
...
// Función para limpiar comentarios y espacios de una línea
...
// Procesar una línea de código ensamblador
void procLinea(string linea, unsigned short& dirActual, int pase) {
    cout << "Procesando línea: " << linea << " en pase " << pase << endl; // Mensaje para indicar el procesamiento de cada línea
    ...
    if (posDosPuntos != string::npos) {
        ...
        if (pase == 1) {
            cout << "Etiqueta encontrada: " << etiqueta << " en dirección " << dirActual << endl; // Mensaje para indicar etiquetas encontradas
    ...
    if (nem == ".ORG") {
        ...
        cout << "Directiva .ORG: cambiando dirección a " << nuevaDir << endl; // Mensaje para indicar cambio de dirección
        dirActual = nuevaDir;
    ...
    if (nem == ".SPACE") {
        ...
        cout << "Directiva .SPACE: reservando espacio de tamaño " << tam << endl; // Mensaje para indicar reserva de espacio
        if(pase == 2) {
    ...
    if (nem == ".WORD") {
        if (pase == 2) {
            ...
            cout << "Directiva .WORD: almacenando valor " << valor << " en dirección " << dirActual << endl; // Mensaje para indicar almacenamiento de valor
    ...
    if (nem == ".STRING") {
        ...
        cout << "Directiva .STRING: almacenando cadena \"" << contenido << "\" en memoria" << endl; // Mensaje para indicar almacenamiento de cadena
        if (pase == 2) {
    ...
    cout << "Procesando instrucción: " << nem << endl; // Mensaje para indicar procesamiento de instrucciones

    if (nem == "MOV") { nem = "ADD"; partes.push_back("R0"); } 
    else if (nem == "IMOV") { nem = "ADDI"; } 
    else if (nem == "NOP") { return; }
    ...
    if (dicInstr.count(nem)) {
        ...
        cout << "Instrucción desconocida: " << nem << endl; // Mensaje para indicar instrucción desconocida
        return;
    }
    ...
    if (tipo == 0) { //tipo R
        cout << "Instrucción tipo R: " << nem << endl; // Mensaje para indicar tipo de instrucción
        int rd = 0, rs1 = 0, rs2 = 0;
        if (nem == "CMP") {
            if(partes.size() > 2) {
                rs1 = obtenerRG(partes[1]);
                rs2 = obtenerRG(partes[2]);
            }
        } else if (nem == "LSL" || nem == "LSR" || nem == "ASR") {
            if(partes.size() > 2) {
                rd = obtenerRG(partes[1]);
                rs1 = obtenerRG(partes[1]);
                rs2 = texto_a_numero(partes[2]);
            }
        } else {
            if(partes.size() > 3) {
                rd = obtenerRG(partes[1]);
                rs1 = obtenerRG(partes[2]);
                rs2 = obtenerRG(partes[3]);
            }
        }
        maquina = (opcode << 12) | (0 << 10) | ((rs1 & 7) << 7) | ((rs2 & 7) << 4) | (rd & 15);
    } 
    else if (tipo == 1) { //tipo I
        cout << "Instrucción tipo I: " << nem << endl; // Mensaje para indicar tipo de instrucción
        int rs1 = 0, inm = 0;
        if(partes.size() > 2) {
            rs1 = obtenerRG(partes[1]);
            inm = texto_a_numero(partes[2]);
        }
        maquina = (opcode << 12) | (1 << 10) | ((rs1 & 7) << 7) | inm;
    } 
    else if (tipo == 2) { //tipo J
        cout << "Instrucción tipo J: " << nem << endl; // Mensaje para indicar tipo de instrucción
        int dir = 0;
        if(partes.size() > 1) {
            dir = texto_a_numero(partes[1]);
        }
        maquina = (opcode << 12) | (2 << 10) | dir;
    }

    if (pase == 2) {
        cout << "Generando código máquina: 0x" << hex << maquina << " en dirección " << dirActual << endl; // Mensaje para indicar generación de código máquina
